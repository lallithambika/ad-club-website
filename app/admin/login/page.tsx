...
function AdminLoginContent() {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      try {
        ...
        if (cancelled) return
        if (!session?.user) {
          console.log("[admin-login] No active session")
          return
        }
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        console.error("[admin-signup] Signup error", data)
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        console.error("[admin-email-login] Sign-in error", error)
      ...
      console.error("[admin-email-login] Unexpected error", err)
      toast.error("An unexpected error occurred")
      setIsLoading(false)
    }
  }
